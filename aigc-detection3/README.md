...
```
> Note: the current implementation calls `.cuda()` directly, so it requires a GPU to run as-is. If the fresh-clone test environment doesn't have one, this will need a quick device-agnostic fix (`.to(device)` with CPU fallback) before it works everywhere.
